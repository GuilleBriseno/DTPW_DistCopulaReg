  ...
  if(!is.null(res$value)){
    ...
    if( !(varying_dependence) ){
      ...
      if( NonSurv_Margin %in% c("logit", "probit", "cloglog", "GEVlink") ){
        ...
        # --- > copula parameter (in copula parameter scale)
        # --- > kendall´s tau (pred.mvt) with confidence interval
        copula_parameter_intercept <- tail(coef(res$value),2)
      ...
      if( NonSurv_Margin %in% c("NBII", "NBI") ){
        ...
        copula_parameter_intercept <- coef(res$value)[(which(names(coef(res$value)) == "(Intercept)")[4]):(length(coef(res$value)))]
        ...
        fit2Sim_cumsum <- sapply(1:ncol(fit1Sim),
                                 function(i)
                                   cumsum( exp(fit1Sim[,i] ) * exp(dat_new$offset)  ),
                                 simplify = TRUE)
        ...
        fit2Sim_cumprod <- sapply(1:ncol(fit1Sim),
                                  function(i)
                                    exp( -  cumsum( exp(fit1Sim[,i] ) * exp(dat_new$offset)  ) ),
                                  simplify = TRUE)
      ...
      if(NonSurv_Margin == "ningunadeestas"){
      ...
      if( NonSurv_Margin %in% c("NBII", "NBI") ){
        ...
        coefficients_copula_parameter <- coef(res$value)[(which(names(coef(res$value)) == "(Intercept)")[4]):(length(coef(res$value)))]
